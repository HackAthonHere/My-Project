...
<body>
    <div class="container">
        <div class="header">
            <header>
                <div class="header-div">
                    ...
                    <nav>
                        <h3><a href="">Home</a></h3>
                        <h3><a href="/http://localhost/insert.php">Insert</a></h3>
        ...
        <center>
            <div class="insert">
                ...
                <form action="" method="post">
                 ...
                 if (isset($_POST['show'])) {
                 $cin=$_POST['cin'];
                 $select=$pdo->prepare("SELECT * FROM stagiaire");
                 $select->execute();
                 while ($rows=$select->fetch(PDO::FETCH_OBJ)) {
                    echo '
                    <tr>
                    <td>'.$rows->cin.'</td>
                    <td>'.$rows->nom.'</td>
                    <td>'.$rows->prenom.'</td>
                    <td>'.$rows->num_filiere.'</td>
                    <td><button type="submit" value="'.$rows->cin.'" name="edite">Edit</button></td>
                    <td><button type="submit" value="'.$rows->cin.'" name="delete">Delete</button></td>
                    </td>
                    </tr>
                    ';
                 }
        ...
        if(isset($_POST["edite"])) {
            $select=$pdo->prepare("SELECT * FROM stagiaire WHERE cin=".$_POST["edite"]);
            $select->execute();
            while ($rows=$select->fetch(PDO::FETCH_OBJ)) {
                print_r($rows);
            }
